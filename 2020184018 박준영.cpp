...
public:
    ...
    bool check_is_a_to_z() {
        ...
        // 문자열 내의 각 문자를 확인하고 알파벳인 경우 해당 비트를 설정한다
        for (auto& c : str) {
            if (std::isalpha(c)) {
...
int main() {
    ...
    std::ofstream out{ "player_list.txt" };
    ...
    double score_avg = (double)score_sum / player_list.size();

    std::cout << "==== Player의 평균 점수 ====" << std::endl;
    std::cout << score_avg << std::endl << std::endl;
    ...
    while (true) {
        ...
        if (std::next(search) != player_list.end()) {
            if (search->get_id() == std::next(search)->get_id()) {
                // 현재 위치의 다다음 위치부터 탐색을 시작한다
                search += 2;
                int move_count = 2;

                // 중복되지 않는 아이디가 발견될 때까지 반복자를 계속 증가시킨다
                while (search != player_list.end()) {
                    if (search->get_id() != search_id)
                        break;
